...
var KingBlingUtils = new function()
{
    ...
    var isWebGlSupportEnabled = function() { 
    	try {
    		var canvas = document.createElement( 'canvas' ); 
    		return !! window.WebGLRenderingContext &&
    			(canvas.getContext('webgl') || canvas.getContext('experimental-webgl') || canvas.getContext('webgl2') || canvas.getContext('experimental-webgl2'));
    	} catch(e) {
    		return false;
    	}
    };
    
    var getBrowserCompatibilityType = function() {
    	var browserVersion = getBrowserVersion();
    	var typeId;
    	if (isWebGlSupportEnabled() && browserVersion.name == 'IE' && browserVersion.version >= 11) {
    		// non html5 and defold
    		typeId = 1
    	} else if (isWebGlSupportEnabled() && 
    		((browserVersion.name == 'Chrome' && browserVersion.version >= 32) || 
    		(browserVersion.name == 'Firefox' && browserVersion.version >= 24) || 
    		(browserVersion.name == 'Edge'))) {
    		// all
    		typeId = 2;
    	} else {
    		// non html5
	...
	var browserCompatibilityType = getBrowserCompatibilityType();
    ...
    return {
        ...
        browserCompatibilityType: browserCompatibilityType
    }
};
...
var CrossPromoBannerAd = function(plataformaUserData)
{
    ...
    var open = function(kingBlingModuleType, targetDiv, options)
    {
        ...
        if (kingBlingModuleType == KingBlingModuleTypeMap.CROSSPROMO_BANNER_TOP) {
            placement = "top";
            templateUrl = 'crossPromoBannerTop.html';
        } else if (kingBlingModuleType == KingBlingModuleTypeMap.CROSSPROMO_BANNER_BOTTOM) {
            placement = "bottom";
            templateUrl = 'crossPromoBannerBottom.html';
        }
        ...
        // default URL
        var url = "/plataforma/canvas/" + templateUrl;
        ...
        $.ajax({
            ...
            success: function(contents) {
                // This will be cleared from 'contents' code
                window.xpbud = mPlataformaUserData;
                $('#' + targetDiv).html(contents);
            },
            dataType: 'html',
            cache: false
        });
